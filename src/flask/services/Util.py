...
class Util:
  ...
  @staticmethod
  def generate_single_message_request_body(recipient: dict, routing_config_id: str) -> dict:
    body = {
      "data": {
        "type": "Message",
        "attributes": {
  ...
  @staticmethod
  def generate_batch_message_request_body(routing_config_id: str, message_batch_reference: str, recipients: dict) -> dict:
    return {
      "data": {
        "type": "MessageBatch",
        "attributes": {
  ...
  @staticmethod
  def generate_jwt(algorithm: str, private_key, headers: dict, payload: dict, expiry_minutes: int = None) -> str:
    if expiry_minutes:
