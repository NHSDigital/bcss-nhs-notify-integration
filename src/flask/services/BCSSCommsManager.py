...
class BCSSCommsManager:
    ...
    def __init__(self):
        ...
        self.data_access = DataAccess(os.getenv("DOCUMENT_DB_URI"), os.getenv("DOCUMENT_DB_NAME"), os.getenv("DOCUMENT_DB_COLLECTION"))

    def send_pre_inviation(self, routing_config_id: str, recipients: list[dict]) -> dict:
        access_token: str = self.auth_manager.get_access_token()
        
        if len(recipients) == 1:
            single_message_response = self.nhs_notify.send_single_message(access_token, routing_config_id, recipients[0])
