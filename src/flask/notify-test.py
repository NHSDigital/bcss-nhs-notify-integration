...
def get_access_token(jwt):
    ...
    token_response = requests.post(config['TOKEN_URL'], data = token_request_body, headers = token_request_headers)
...
def create_message_batch():
    ...
    notify_response = requests.post(config['NOTIFY_INT_URL']+"/v1/message-batches", data = json.dumps(message_batch_body), headers = notify_headers)
...
def get_message_status():
    if request.method == 'POST':
          ...
          notify_response = requests.get(config['NOTIFY_INT_URL']+"/v1/messages/" + message_id, headers = notify_headers)
          return render_template(
            'jsonresponse.html',
            responseCode = notify_response.status_code,
            responseJSON = json.dumps(notify_response.json(), indent=4)
...
@app.route('/get_nhs_app_details', methods=['GET','POST'])    ####### STARTING POINT FOR TOMORROW
def get_nhs_app_details():
    if request.method == 'POST':
          ...
          notify_response = requests.get(config['NOTIFY_INT_URL']+"/channels/nhsapp/accounts", headers = notify_headers, params = params)
          return render_template(
            'jsonresponse.html',
            responseCode = notify_response.status_code,
            responseJSON = json.dumps(notify_response.json(), indent=4)
